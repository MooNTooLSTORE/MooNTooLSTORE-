...
export async function GET(request: Request) {
  ...
  try {
    ...
    const campaigns = await collection.find({}).sort({ createdAt: -1 }).skip(skip).limit(limit).toArray();
...
async function sendCampaign(campaignId: ObjectId) {
    let redisClient: any;
    let mongoClient: MongoClient | undefined;
    const { TELEGRAM_TOKEN, MONGODB_URI, MONGODB_DB_NAME, REDIS_URI } = await getConfig();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === 'restart' && body.campaignId) {
        sendCampaign(new ObjectId(body.campaignId));
    ...
    sendCampaign(result.insertedId);
...
export async function DELETE(request: Request) {
    ...
    try {
        ...
        const result = await db.collection(CAMPAIGNS_COLLECTION).deleteOne({ _id: new ObjectId(campaignId) });
