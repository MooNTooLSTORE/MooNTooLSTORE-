...
export default function ActivatePageClient() {
  ...
  const handleActivate = async (activation_key: string) => {
    ...
    setError(null);
    setActivationStep('Запуск процесса активации...');
    
    try {
        // Единственный запрос от клиента, который запускает всю цепочку на сервере
        ...
        if (!response.ok || !data.success) {
            throw new Error(data.message || 'Произошла неизвестная ошибка во время активации.');
        }

        setActivationStep('Активация успешна! Перезагрузка...');
        toast({
            title: "Активация завершена",
            description: "Приложение перезагружается для применения изменений.",
        });
        
        // Надежное перенаправление через перезагрузку страницы.
        setTimeout(() => {
