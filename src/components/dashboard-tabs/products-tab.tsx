...
export function ProductsTab({
  ...
  const currencyDisplay = telegramProviderToken ? telegramPaymentCurrency : "⭐";
      ...
      <TableBody>
        {productList.length > 0 ? productList.map((product) => (
          <TableRow key={product._id}>
            ...
            <TableCell>{product.priceReal || 0}</TableCell>
            <TableCell>{product.price || 0}</TableCell>
            <TableCell>{product.type === 'static' ? getKeyCount(product.staticKey) : '∞'}</TableCell>
          ...
          <TableRow>
            <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">
              {currentCategoryView ? "В этой категории еще нет товаров." : "Нет товаров без категории."}
            </TableCell>
          </TableRow>
        )}
      </TableBody>
        ...
        <ScrollArea className="h-[500px]">
          {currentCategoryView === null ? (
            <div>
              ...
              {renderProductTable(productsWithoutCategory)}
            </div>
          ) : (
            renderProductTable(productsInCategory)
          )}
        </ScrollArea>
      </CardContent>
    </Card>
