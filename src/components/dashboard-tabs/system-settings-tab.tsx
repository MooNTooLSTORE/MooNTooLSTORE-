...
export function SystemSettingsTab({
  ...
  const handleDownloadBackup = () => {
    if (tgBgExportStatus.filePath) {
      // The file path is absolute on the server. We need to trigger a download
      // by making the browser navigate to a special download link.
      // We assume the backups are served statically from a 'backups' directory.
      const fileName = tgBgExportStatus.filePath.split('/').pop();
      window.location.href = `/api/download?file=backups/${fileName}`;
    }
  }
