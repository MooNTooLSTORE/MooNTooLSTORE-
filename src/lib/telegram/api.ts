...
export async function apiCall(token: string, method: string, payload: any) {
    ...
    try {
        ...
        // We won't log errors here to avoid duplicate logging, let the calling function handle it.
        if (!result.ok) {
            console.error(`Telegram API Error (${method}):`, result.description, 'Payload:', payload);
        }
        return result;
    } catch (error) {
...
export async function getMe(token: string) {
    const url = `${BASE_URL(token)}/getMe`;
    try {
        const response = await fetch(url);
        return response.json();
    } catch (error) {
        console.error(`Failed to call Telegram API method getMe:`, error);
        return { ok: false, error };
    }
}
...
export function sendInvoice(token: string, chatId: number, title: string, description: string, payload: string, providerToken: string | undefined, currency: string, prices: any[], replyMarkup?: any) {
...
export async function setBotCommands(token: string) {
    const commands = [
        ...
        { command: 'supportpanel', description: 'Панель управления поддержкой (для модераторов)' },
    ];
